...
if __name__ == "__main__":
    ...
    model = RepCPSI()
    # model = SpaSpeConv(in_channels=3, out_channels=3)
    model.eval()
    output_tensor = model(input_tensor)
    print(output_tensor.size(), output_tensor.mean())
    ...
    for m in model.modules():
        if hasattr(m, 'perform_regroup'):
            m.perform_regroup()
    output_tensor2 = model(input_tensor)
    print(output_tensor2.size(), output_tensor2.mean())
    print('Parameters number is ', sum(param.numel() for param in model.parameters()))
    ...
    print('difference between the outputs of the training-time and converted module is')
    print(((output_tensor - output_tensor2) ** 2).sum())
